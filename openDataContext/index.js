    ...
    if (data.command == 'open') {
      ...
      wx.getFriendCloudStorage({
        ...
        success: res => {
          ...
          // 比较用户的score和maxScore
          console.log(currentUser.KVDataList[0].value)
          if (!currentUser.KVDataList[0] || Number(currentUser.KVDataList[0].value) < Number(data.score) ) {
            console.log(data.score)
            wx.setUserCloudStorage({
              KVDataList: [{key: 'maxScore', value: data.score}]
            })
          }
          // 保存比较大的score
